...
import { ArrowLeft, Camera, Check, CheckCircle, ChevronDown, ChevronUp, Mail, Minus, Save, X } from "lucide-react";
...
function SafetyRoundDetailsContent() {
    ...
    const handleUpdateItem = (itemId: string, updates: Partial<SafetyRoundItem>) => {
        if (!round) return;
        const newItems = round.items.map((item) => (item.id === itemId ? { ...item, ...updates } : item));
    ...
    const statusColors = (status: SafetyRoundItem["status"]) => {
        if (status === "OK") return { bg: "#dcfce7", fg: "#166534", border: "#bbf7d0" };
        if (status === "Avvik") return { bg: "#fee2e2", fg: "#991b1b", border: "#fecaca" };
    ...
    return (
        <main className="container page-shell">
            <div className="flex-between" style={{ marginBottom: "1rem", gap: "1rem", flexWrap: "wrap" }}>
                <Link href={`/projects/safety-rounds?projectId=${project.id}`} className="back-link" style={{ marginBottom: 0 }}>
            ...
            <div id="safety-round-content" style={{ backgroundColor: "white", padding: "1rem" }}>
                ...
                <div style={{ display: "grid", gap: "2rem" }}>
                    {Object.entries(groupedItems).map(([category, items]) => (
                        <div key={category}>
                            <h3 style={{ borderBottom: "1px solid var(--border)", paddingBottom: "0.5rem", marginBottom: "1rem" }}>{category}</h3>
                                        ...
                                        <div key={item.id} className="card" style={{ padding: "1rem" }}>
                                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "1rem" }}>
                                                ...
                                                <div style={{ display: "flex", gap: "0.5rem" }}>
                                                    ...
                                                    <button className="btn btn-ghost" onClick={() => setExpandedItem(expandedItem === item.id ? null : item.id)}>
                                                    ...
                                                    {isSigned ? (
                                                        <>
                                                            ...
                                                            {item.photoUrl && (
                                                                ...
                                                                <img src={item.photoUrl} alt="Dokumentasjon" style={{ maxWidth: "100%", borderRadius: "8px", marginTop: "0.5rem" }} />
                                                            )}
                                                        </>
                                                    ) : (
                                                        <div style={{ display: "grid", gap: "1rem" }}>
